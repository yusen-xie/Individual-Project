	...
	<div class="larry-personal">
		...
		<div class="layui-inline">
		    <div class="layui-input-inline" style="float:left;border:0px solid black;">
		      <form action="<%=path %>/admin/dingdanList.do" method="post" id="searchform" name="searchform">
                ...
                <div class="layui-form news_list" >
					...
					<tbody class="news_content">
					   <c:forEach items="${pageInfo.list}" var="dingdan">
						<tr>
						    <td>
						    <input name="checked" lay-skin="primary" lay-filter="choose" type="checkbox" value="${dingdan.id}">
						    <div class="layui-unselect layui-form-checkbox" lay-skin="primary"><i class="layui-icon"></i></div>
						    </td>
							<td>${dingdan.ddno}</td>
							<td>${dingdan.member.uname}</td>
                    ...
                    <c:if test="${fn:length(pageInfo.list) gt 0}">
				              ...
				              <ul>
				                  <li><a href="dingdanList.do?pageNum=${pageInfo.hasPreviousPage==false?1:pageInfo.prePage}&key=${key}&key1=${key1}">上一页</a></li>
				                  <c:forEach begin="1" end="${pageInfo.pages}" varStatus="status">
				                      <li class="pagesz ${status.count eq pageInfo.pageNum ?"acvtive":""}" ><a href="dingdanList.do?pageNum=${status.count}&key=${key}&key1=${key1}" >${status.count}</a></li>
				                  </c:forEach>
				                  <li><a href="dingdanList.do?pageNum=${pageInfo.hasNextPage==false? pageInfo.pages : pageInfo.nextPage}&key=${key}&key1=${key1}">下一页</a></li>
				              </ul>
				          </div>       
	            		</c:if> 
                    </td>
...
<script type="text/javascript">
	...
	function deleteAll(){
    	...
    	layer.confirm('是否确认删除？', {
            ...
            $.ajax({
      		type:"post",
      		url:"dingdanDelAll.do?vals="+vals,
      		date:"",
      		success:function(msg){
      			location.replace("dingdanList.do");
      		  }
            })
      	  }else{
				layer.msg("请选择要删除的项");
